...
class Instaloader:
    ...
    def download_pic(self, filename: str, url: str, mtime: datetime,
        ...
        if os.path.isfile(filename):
            self.context.log(filename + ' ja existe', end=' ', flush=True)
            return False
    ...
    def login(self, user: str, passwd: str, proxies=None) -> None:
    ...
    def download_profiles(self, profiles: Set[Profile],
        ...
        for profile in profiles:
            with error_handler(profile.username):  # type: ignore # (ignore type for Python 3.5 support)
                ...
                if posts:
                    self.context.log("Recuperando posts do perfil {}.".format(profile_name))
        ...
        if stories and profiles:
            with self.context.error_catcher("Baixar stories"):
                self.context.log("Baixando stories")
    ...
    def download_profile(self, profile_name: Union[str, Profile],
        ...
        if self.context.is_logged_in and profile.has_blocked_viewer and not profile.is_private:
            ...
            raise ProfileNotExistsException("Perfil {} bloqueou voce".format(profile_name))

        # Download profile picture
        if profile_pic or profile_pic_only:
            with self.context.error_catcher('Baixando imagem de perfil de {}'.format(profile_name)):
                self.download_profilepic(profile)
        if profile_pic_only:
            return
        ...
        if profile.is_private:
            if not self.context.is_logged_in:
                raise LoginRequiredException("perfil %s necessita de login" % profile_name)
            if not profile.followed_by_viewer and \
                    self.context.username != profile.username:
                raise PrivateProfileNotFollowedException("Perfil %s: privado mas nao seguido." % profile_name)
        else:
            if self.context.is_logged_in and not (download_stories or download_stories_only):
                self.context.log("perfil %s tambem poderia ser baixado anonimamente." % profile_name)

        # Download stories, if requested
        if download_stories or download_stories_only:
            if profile.has_viewable_story:
                with self.context.error_catcher("Baixando stories de {}".format(profile_name)):
                ...
                self.context.log("{} nao possui stories.".format(profile_name))
        if download_stories_only:
            return

        # Download tagged, if requested
        if download_tagged or download_tagged_only:
            with self.context.error_catcher('Baixando tagged de {}'.format(profile_name)):
                self.download_tagged(profile, fast_update=fast_update, post_filter=post_filter)
    ...
    def interactive_login(self, username: str) -> None:
        ...
        if self.context.quiet:
            raise LoginRequiredException("Modo silencioso requer senha ou arquivo de sessão válido.")
        try:
            password = None
            while password is None:
                password = getpass.getpass(prompt="Digite a senha do Instagram para %s: " % username)
                try:
                    self.login(username, password)
                except BadCredentialsException as err:
                    print("Erro de credencial no Instagram:", err, file=sys.stderr)
                    password = None
        except TwoFactorAuthRequiredException:
            while True:
                try:
                    code = input("Digite o codigo de verificacao 2FA: ")
